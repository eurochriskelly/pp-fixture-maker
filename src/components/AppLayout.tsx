...
export default function AppLayout() {
  ...
  const getBreadcrumbs = () => {
    ...
    if (parts[0] === 'competition' && parts[1]) {
      ...
      if (comp) {
        items.push({
          label: comp.name,
          path: `/competition/${compId}/groups`
        });
      } else {
        items.push({
  ...
  // Show only hero section when no tournament is open
  if (!currentTournament && !isPrintMode) {
    return <HeroSection />;
  }

  // Print mode - no layout
  if (isPrintMode) {
    return (
                  ...
                  <SidebarMenuItem>
                    ...
                    <CollapsibleContent>
                      <SidebarMenuSub>
                        ...
                        <SidebarMenuSubItem>
                          <SidebarMenuSubButton asChild isActive={location.pathname === "/reports/locations"}>
                            <Link to="/reports/locations">
                              <span>Locations report</span>
                            </Link>
                          </SidebarMenuSubButton>
                        </SidebarMenuSubItem>
                      </SidebarMenuSub>
